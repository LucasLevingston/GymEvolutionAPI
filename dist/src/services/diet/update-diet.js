...
async function updateDiet(dietId, data) {
    try {
        ...
        if (meals && Array.isArray(meals) && meals.length > 0) {
            ...
            if (mealsToDelete.length > 0) {
                await prisma_1.prisma.meal.deleteMany({
                    where: {
                        id: {
                            in: mealsToDelete,
                        },
        ...
        if (diet?.userId) {
            await (0, create_history_entry_1.createHistoryEntry)(diet.userId, `Diet for week ${dietData.weekNumber} updated`);
        }
        // Fetch the updated diet with all related data
        const finalDiet = await prisma_1.prisma.diet.findUnique({
            where: { id: dietId },
            include: {
                meals: {
                    include: {
...
async function updateMeal(meal) {
    ...
    if (mealItems && Array.isArray(mealItems) && mealItems.length > 0) {
        ...
        if (itemsToDelete.length > 0) {
            await prisma_1.prisma.mealItems.deleteMany({
                where: {
                    id: {
                        in: itemsToDelete,
                    },
...
async function createMeal(dietId, meal) {
    const { mealItems, ...mealData } = meal;
    // Create the meal
    const newMeal = await prisma_1.prisma.meal.create({
        data: {
            name: mealData.name,
            calories: mealData.calories || 0,
            protein: mealData.protein || 0,
...
async function updateMealItem(item) {
    const { substitutions, ...itemData } = item;
    // Update the meal item
    const updatedItem = await prisma_1.prisma.mealItems.update({
        where: { id: item.id },
        data: {
            name: itemData.name,
    ...
    if (substitutions && Array.isArray(substitutions) && substitutions.length > 0) {
        ...
        const existingSubIds = await prisma_1.prisma.mealItems.findMany({
...
async function createMealItem(mealId, item) {
    const { substitutions, ...itemData } = item;
    // Create the meal item
    const newItem = await prisma_1.prisma.mealItems.create({
        data: {
            name: itemData.name,
            quantity: itemData.quantity,
            quantityType: itemData.quantityType,
